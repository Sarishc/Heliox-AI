...
- `NEXT_PUBLIC_BETA_ACCESS_CODE` - Optional access code for private beta
  ...
  - `/login` and `/signup` are public placeholders for now

## ✅ TODOs (Product UI Gaps)

- TODO: Add usage analytics page for `/api/v1/usage/*` endpoints.
- TODO: Add teams management page for `/api/v1/teams`.
- TODO: Add job explorer page for `/api/v1/jobs` (requires auth).
- TODO: Add settings page for alert configuration (`/api/v1/alert-settings`).
- TODO: Replace beta access gate with full auth when backend auth is enabled.
