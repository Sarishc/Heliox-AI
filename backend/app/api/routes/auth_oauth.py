...
async def google_oauth_start(*, db: Session = Depends(get_db), request_data: OAuthStartRequest):
    """
...
async def google_oauth_callback(
    ...
    frontend_redirect = (
        ...
        or state_data.get("redirect_uri")
    ...
    try:
        ...
        if not check_domain_allowed(email, team):
            logger.warning(f"OAuth login blocked for {email} - domain not allowed for team {team.id}")
