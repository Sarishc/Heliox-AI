    ...
    if not settings.STRIPE_SECRET_KEY:
        raise HTTPException(
            ...
            detail=(
                ...
                "Contact your administrator to set up Stripe."
            ),
        )
...
@router.post("/upgrade", response_model=CreateCheckoutSessionResponse, summary="Start Stripe Checkout to upgrade to Growth")
        ...
        if event_type == "customer.subscription.created":
            ...
            if subscription_id:
                try:
                    ...
                    logger.info(
                        f"invoice.paid: synced subscription {subscription_id} "
            ...
            if subscription_id:
                ...
                if team_sub:
                    ...
                    logger.info(
                        ...
                        f"(team {team_sub.team_id})"
                    )
                else:
                    logger.warning(
                        f"invoice.payment_failed: no local subscription found for {subscription_id}"
                    )
