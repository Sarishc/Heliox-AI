...
@router.get(
    ...
    summary="Get usage summary for a date range",
)
def get_usage_summary(
    *,
    ...
    rollup_query = (
        ...
        .group_by(UsageDailyRollup.event_type)
        .all()
    )

    # Build breakdown
    breakdown = []
    totals = {"api_requests": 0, "ingestion_line_items": 0, "seats": 0, "gpu_nodes": 0}
...
def get_current_month_usage(
    *,
    ...
    return get_usage_summary(db=db, auth=auth, from_date=first_day_of_month, to_date=today)
...
def get_recent_usage_events(
    *,
    ...
    """
    query = db.query(UsageEvent).filter(UsageEvent.team_id == auth.team_id)
