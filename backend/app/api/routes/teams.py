...
from app.core.plan_enforcement import check_team_limit, require_plan_for_team
...
@router.get("/{team_id}", response_model=Team, summary="Get a team by ID")
def read_team(
    *,
    db: Session = Depends(get_db),
    team_id: UUID,
...
@router.put("/{team_id}", response_model=Team, summary="Update team details")
def update_team(
    *,
    db: Session = Depends(get_db),
    team_id: UUID,
...
@router.put("/{team_id}/budget", response_model=Team, summary="Update team budget limit")
