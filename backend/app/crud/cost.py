...
class CRUDCostSnapshot(TenantScopedMixin, CRUDBase[CostSnapshot, CostSnapshotCreate, None]):
    ...
    def get_multi(self, db: Session, *, skip: int = 0, limit: int = 100, team_id) -> List[CostSnapshot]:
        return db.query(CostSnapshot).filter(CostSnapshot.team_id == team_id).offset(skip).limit(limit).all()

    def get_by_date_range(self, db: Session, *, start_date: date, end_date: date, team_id) -> List[CostSnapshot]:
        """
    ...
    def get_by_provider(
        self, db: Session, *, provider: str, start_date: date, end_date: date, team_id
    ) -> List[CostSnapshot]:
        """
    ...
    def get_total_cost(self, db: Session, *, start_date: date, end_date: date, team_id) -> float:
        """
...
class CRUDUsageSnapshot(TenantScopedMixin, CRUDBase[UsageSnapshot, UsageSnapshotCreate, None]):
    ...
    def get_multi(self, db: Session, *, skip: int = 0, limit: int = 100, team_id) -> List[UsageSnapshot]:
        return db.query(UsageSnapshot).filter(UsageSnapshot.team_id == team_id).offset(skip).limit(limit).all()

    def get_by_date_range(self, db: Session, *, start_date: date, end_date: date, team_id) -> List[UsageSnapshot]:
        """
    ...
    def get_total_hours(self, db: Session, *, start_date: date, end_date: date, team_id) -> float:
        """
        ...
        """
        result = (
            db.query(func.sum(UsageSnapshot.gpu_hours))
            .filter(
                and_(
                    UsageSnapshot.team_id == team_id,
                    UsageSnapshot.date >= start_date,
