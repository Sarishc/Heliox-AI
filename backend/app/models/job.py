...
class Job(Base, UUIDMixin, TimestampMixin):
    ...
    # Job Configuration
    model_name: Mapped[str] = mapped_column(String(255), nullable=False, comment="Name of the ML model being executed")

    gpu_type: Mapped[str] = mapped_column(
        String(100), nullable=False, comment="Type of GPU used (e.g., A100, H100, V100)"
    )
    ...
    environment: Mapped[Optional[str]] = mapped_column(
        String(100), nullable=True, comment="Environment (e.g., prod, staging, dev)"
    )

    project: Mapped[Optional[str]] = mapped_column(
        String(120), nullable=True, comment="Project or cost center identifier"
    )

    # Timing
    start_time: Mapped[Optional[datetime]] = mapped_column(nullable=True, comment="When the job started execution")

    end_time: Mapped[Optional[datetime]] = mapped_column(nullable=True, comment="When the job completed or failed")
    ...
    # Relationships
    team: Mapped["Team"] = relationship("Team", back_populates="jobs", lazy="selectin")
    ...
    def __repr__(self) -> str:
        return f"<Job(id={self.id}, model={self.model_name}, " f"gpu={self.gpu_type}, status={self.status})>"
