...
class OAuthIdentity(Base, UUIDMixin, TimestampMixin):
    ...
    provider_user_id: Mapped[str] = mapped_column(
        String(255), nullable=False, index=True, comment="User ID from OAuth provider"
    )

    # Email and verification
    email: Mapped[str] = mapped_column(String(255), nullable=False, index=True, comment="Email from OAuth provider")
    ...
    name: Mapped[Optional[str]] = mapped_column(String(255), nullable=True, comment="Full name from OAuth provider")

    picture: Mapped[Optional[str]] = mapped_column(
        String(512), nullable=True, comment="Profile picture URL from OAuth provider"
    )
    ...
    access_token_encrypted: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Fernet-encrypted OAuth access token"
    )

    refresh_token_encrypted: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Fernet-encrypted OAuth refresh token"
    )

    token_expires_at: Mapped[Optional[datetime]] = mapped_column(nullable=True, comment="When access token expires")

    # Last login tracking
    last_login_at: Mapped[Optional[datetime]] = mapped_column(
        nullable=True, index=True, comment="Last successful OAuth login"
    )
    ...
    def __repr__(self) -> str:
        return f"<OAuthIdentity(user_id={self.user_id}, provider={self.provider.value}, " f"email={self.email})>"
