...
class RecommendationEvidence(BaseModel):
    ...
    # Common fields
    date_range: Optional[Dict[str, str]] = Field(None, description="Date range for the evidence (start_date, end_date)")
...
class Recommendation(BaseModel):
    ...
    action_status: Optional[str] = Field(None, description="User action status: applied, dismissed, or None (pending)")
    ...
    severity: RecommendationSeverity = Field(..., description="Severity level (low, medium, high)")

    estimated_savings_usd: float = Field(0.0, ge=0, description="Estimated cost savings in USD if implemented")

    evidence: RecommendationEvidence = Field(..., description="Structured evidence supporting this recommendation")
...
class RecommendationFilters(BaseModel):
    ...
    min_severity: Optional[RecommendationSeverity] = Field(None, description="Minimum severity to include")

    types: Optional[List[RecommendationType]] = Field(None, description="Filter by recommendation types")

    team_id: Optional[UUID] = Field(None, description="Filter by team ID")

    min_savings: Optional[float] = Field(None, ge=0, description="Minimum estimated savings to include")
...
class RecommendationResponse(BaseModel):
    ...
    summary: Dict[str, Any] = Field(default_factory=dict, description="Summary statistics about recommendations")

    date_range: Dict[str, str] = Field(..., description="Date range used for analysis")

    total_estimated_savings_usd: float = Field(0.0, description="Sum of all estimated savings")
