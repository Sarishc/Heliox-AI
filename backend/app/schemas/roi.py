...
class SavingsByCategory(BaseModel):
    ...
    type: str = Field(
        ...,
        description="Recommendation type (idle_gpu, long_running_job, off_hours_usage)",
    )
...
class ROIDashboardResponse(BaseModel):
    ...
    savings_by_category: List[SavingsByCategory] = Field(
        default_factory=list, description="Savings grouped by recommendation type"
    )
    top_recommendations: List[TopRecommendation] = Field(
        default_factory=list,
        description="Top 5 savings opportunities by estimated value",
    )
    provider_breakdown: List[ProviderBreakdown] = Field(default_factory=list, description="Cost by cloud provider")
