...
class CostIngestionService:
    ...
    @staticmethod
    def _resolve_team_id(record_team_id: Optional[str], default_team_id: Optional[str]) -> UUID:
    ...
    @staticmethod
    def normalize_record(record: CostDataRecord, default_team_id: Optional[str] = None) -> Dict:
        """
    ...
    def parse_csv_file(csv_content: str) -> List[CostDataRecord]:
        ...
        try:
            ...
            for row_num, row in enumerate(reader, start=2):  # Start at 2 (header is row 1)
                try:
                    ...
                    try:
                        ...
                        if cost_value <= 0:
                            errors.append(f"Row {row_num}: cost_usd must be positive, got {cost_str}")
                            continue
                    except (InvalidOperation, ValueError):
                        errors.append(f"Row {row_num}: Invalid cost_usd value '{cost_str}'. Expected decimal number")
                        continue
            ...
            if errors and not records:
                ...
                raise ValueError(f"CSV parsing failed with {len(errors)} error(s). " f"First error: {errors[0]}")
    ...
    def ingest_cost_data(self, file_path: Optional[Path] = None, team_id: Optional[str] = None) -> IngestionResult:
        """
        ...
        try:
            ...
            logger.info(
                f"Ingestion complete: {result.inserted} inserted, " f"{result.updated} updated, {result.failed} failed"
            )
    ...
    def ingest_cost_data_from_csv(self, csv_content: str, team_id: Optional[str] = None) -> IngestionResult:
        """
    ...
    def ingest_cost_records(self, *, records: List[CostDataRecord], team_id: Optional[str] = None) -> IngestionResult:
        """
