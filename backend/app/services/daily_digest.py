...
class DailyDigestGenerator:
    ...
    def _get_cost_for_period(self, start_date: date, end_date: date) -> float:
        """
    ...
    def _get_top_gpu_types(self, start_date: date, end_date: date, limit: int = 5) -> List[Dict]:
        """
        ...
        query = (
            ...
            .where(CostSnapshot.date >= start_date, CostSnapshot.date <= end_date)
    ...
    def generate_daily_digest(self, target_date: date = None) -> DailyDigestPayload:
        """
        ...
        filters = RecommendationFilters(start_date=yesterday - timedelta(days=14), end_date=yesterday)
        ...
        sorted_recs = sorted(all_recommendations, key=lambda x: x.estimated_savings_usd, reverse=True)[:5]
        ...
        global_potential_savings = sum(rec.estimated_savings_usd for rec in all_recommendations)

        # Format top GPU types as "models" for schema compatibility
        # (Schema expects top_models, but we're providing top GPU types)
        global_top_models = [{"model_name": item["gpu_type"], "cost": item["cost"]} for item in global_top_gpu_types]
