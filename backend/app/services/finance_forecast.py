...
class FinanceForecastService:
    ...
    def _fetch_daily_costs(self, team_id: UUID, start_date: date, end_date: date) -> List[Tuple[date, float]]:
    ...
    def _forecast_daily_costs(self, daily_costs: List[float], *, method: str, horizon_days: int) -> List[float]:
    ...
    def _cost_breakdown(
        ...
        daily_costs = {str(day): total for day, total in self._fetch_daily_costs(team_id, start_date, end_date)}
        if not daily_costs:
            return [], 0.0
