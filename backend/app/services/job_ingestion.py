...
class JobIngestionService:
    ...
    async def load_mock_data(self) -> MockJobsData:
        ...
        try:
            ...
            logger.info(f"Validated mock data: {len(mock_data.teams)} teams, " f"{len(mock_data.jobs)} jobs")
            return mock_data
    ...
    async def get_or_create_team(self, team_data: TeamData) -> Team:
        ...
        result = await self.db_session.execute(select(Team).where(Team.name == team_data.name))
    ...
    async def ingest_jobs(self) -> Dict[str, Any]:
        ...
        logger.info(f"Teams processed: {teams_created} created, {teams_existing} existing")
        ...
        for job_data in mock_data.jobs:
            try:
                ...
                result = await self.db_session.execute(select(Job).where(Job.job_id == job_data.job_id))
