...
def run_integration_sync(self, connection_id: str, sync_run_id: str):
    ...
    try:
        ...
        connection = db.query(IntegrationConnection).filter(IntegrationConnection.id == UUID(connection_id)).first()

        if not connection:
            logger.error(f"Connection {connection_id} not found")
            return

        sync_run = db.query(IntegrationSyncRun).filter(IntegrationSyncRun.id == UUID(sync_run_id)).first()
        ...
        try:
            ...
            logger.error(
                f"Integration sync failed for {connection_id}: {sync_error}",
                exc_info=True,
            )
