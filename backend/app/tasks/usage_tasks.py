...
from app.models.usage import UsageEvent, UsageDailyRollup, UsageEventType
...
def rollup_daily_usage(date_str: str = None) -> Dict:
    ...
    try:
        ...
        for row in rollup_query:
            ...
            # Upsert daily rollup
            existing_rollup = db.query(UsageDailyRollup).filter(
        ...
        logger.info(
            f"Daily usage rollup completed for {target_date}: "
            f"{rollups_created} created, {rollups_updated} updated"
        )
...
def cleanup_old_daily_rollups(retention_months: int = 12) -> Dict:
    ...
    try:
        ...
        # Delete old rollups
        deleted_count = db.query(UsageDailyRollup).filter(
            UsageDailyRollup.date < cutoff_date
        ).delete(synchronize_session=False)
...
def snapshot_daily_seats() -> Dict:
    ...
    try:
        ...
        team_seats = db.query(
            ...
            func.count(TeamMember.id).label('seat_count')
        ).outerjoin(
            TeamMember,
        ...
        for row in team_seats:
            ...
            record_seat_snapshot(
                team_id=team_id,
                active_users_count=seat_count,
                db=db
            )
        ...
        logger.info(
            f"Daily seats snapshot completed: {teams_processed} teams, "
