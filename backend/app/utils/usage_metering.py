...
def record_usage_event(
    ...
    try:
        ...
        logger.debug(f"Recorded usage event: team={team_id}, type={event_type.value}, " f"quantity={quantity}")
...
def record_ingestion_event(team_id: UUID, line_items_count: int, source: str, db: Optional[Session] = None) -> None:
    """
...
def record_seat_snapshot(team_id: UUID, active_users_count: int, db: Optional[Session] = None) -> None:
    """
...
def record_gpu_node_snapshot(team_id: UUID, gpu_nodes_count: int, db: Optional[Session] = None) -> None:
    """
