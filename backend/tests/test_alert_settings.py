...
def test_create_alert_settings(db_session, sample_team):
    ...
    # Verify
    retrieved = db_session.query(AlertSettings).filter(AlertSettings.team_id == sample_team.id).first()
...
def test_delete_team_cascades_settings(db_session, sample_team):
    ...
    retrieved = db_session.query(AlertSettings).filter(AlertSettings.id == settings_id).first()
