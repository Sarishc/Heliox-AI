...
def test_forecast_usage_shape(db_with_usage_data, sample_team):
    ...
    result = service.forecast_usage(team_id=sample_team.id, provider="aws", gpu_type="a100", horizon_days=7)
...
def test_forecast_spend_shape(db_with_cost_data, sample_team):
    ...
    result = service.forecast_spend(team_id=sample_team.id, provider="aws", gpu_type="a100", horizon_days=7)
...
def test_forecast_confidence_bands_widen(db_with_usage_data, sample_team):
    ...
    first_band_width = result["forecast"][0]["upper_bound"] - result["forecast"][0]["lower_bound"]
    last_band_width = result["forecast"][-1]["upper_bound"] - result["forecast"][-1]["lower_bound"]
...
def test_forecast_caching(db_with_usage_data, sample_team):
    ...
    result1 = service.forecast_usage(team_id=sample_team.id, provider="aws", gpu_type="a100", horizon_days=7)
    result2 = service.forecast_usage(team_id=sample_team.id, provider="aws", gpu_type="a100", horizon_days=7)
